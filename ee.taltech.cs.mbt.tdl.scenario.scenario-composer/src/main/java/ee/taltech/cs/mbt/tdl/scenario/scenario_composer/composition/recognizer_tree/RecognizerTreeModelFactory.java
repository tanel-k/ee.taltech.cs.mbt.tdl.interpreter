...
import ee.taltech.cs.mbt.tdl.scenario.scenario_composer.composition.recognizer_tree.base_factory.RecognizerTreeModelBaseSystemFactory;
...
public class RecognizerTreeModelFactory extends RecognizerTreeModelBaseSystemFactory {
	...
	private void prepareConstructionContext() {
		...
		parameters.getTdlExpression().getRootNode().accept(new BaseBooleanNodeVisitor<Void>() {
			...
			private Void visitQuantification(boolean universal, AbsTrapsetQuantifierNode node) {
				...
				AbsTrapsetEvaluation<?> trapset = parameters.getTrapsetEvaluationMap()
